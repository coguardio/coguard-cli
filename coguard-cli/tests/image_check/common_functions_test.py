...
from src.auth import DealEnum
from src.image_check.config_file_finders.config_file_finder_nginx \
...
class TestCommonImageCheckingFunc(unittest.TestCase):
    ...
    def test_create_zip_to_upload_docker_image_docker_creation_failed(self):
        ...
        with unittest.mock.patch(
                "src.image_check.docker_dao.create_docker_image",
    ...
    def test_create_zip_to_upload_docker_image_get_inspect_result_failed(self):
        ...
        with unittest.mock.patch(
             ...
             unittest.mock.patch(
                "src.image_check.docker_dao.get_inspect_result",
    ...
    def test_create_zip_to_upload_docker_image_get_file_store_failed(self):
        ...
        with unittest.mock.patch(
             ...
             unittest.mock.patch(
                "src.image_check.docker_dao.store_image_file_system",
                 new_callable = lambda: lambda x: None):
            result = image_check.create_zip_to_upload_from_docker_image(
                "foo",
                "foo",
        ...
        with unittest.mock.patch(
             ...
             unittest.mock.patch(
                "src.image_check.docker_dao.store_image_file_system",
                 new_callable = lambda: lambda x: "/foo/bar"), \
             unittest.mock.patch(
                "src.image_check.find_configuration_files_and_collect",
    ...
    def test_create_zip_to_upload_docker_image_collected(self):
        ...
        with unittest.mock.patch(
             ...
             unittest.mock.patch(
                 "src.image_check.docker_dao.rm_temporary_container_name"
             ):
            result = image_check.create_zip_to_upload_from_docker_image(
                "foo",
                "foo",
    ...
    def test_find_configuration_files_and_collect_none_result(self):
        ...
        """
        with unittest.mock.patch(
                'src.image_check.config_file_finder_factory.config_file_finder_factory',
                new_callable=lambda: lambda: [ConfigFileFinderNginx()]), \
             unittest.mock.patch(
                ('src.image_check.config_file_finders.config_file_finder_nginx.'
                 'ConfigFileFinderNginx.find_configuration_files'),
                 new_callable=lambda: lambda x, y, z: []), \
             unittest.mock.patch(
                ('src.image_check.extract_docker_file_and_store'),
                 new_callable=lambda: lambda x: None):
    ...
    def test_extract_docker_file_and_store_none(self):
        ...
        """
        with unittest.mock.patch(
                'src.image_check.docker_dao.extract_docker_file',
                new_callable=lambda: lambda x: None):
